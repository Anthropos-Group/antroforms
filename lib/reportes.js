...
async function obtenerReporte(pool, { desde, hasta, encuestadorId, mesGestion, pdv, limit } = {}) {
  ...
  if (mesGestion) {
    ...
    condiciones.push(`cc.mes_gestion = $${valores.length}`);
  }
  if (pdv) {
    valores.push(pdv);
    condiciones.push(`trim(cc.pdv) = $${valores.length}`);
  }
